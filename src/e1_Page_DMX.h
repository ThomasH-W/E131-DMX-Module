...
void send_DMX_configuration_html()
{
	...
	{
		e1c.e1config.DMXoutputOn = false;
		String temp = "";
		for (uint8_t i = 0; i < server.args(); i++)
		{
			...
			if (server.argName(i) == "deviceNum")
				e1c.e1config.deviceNum = server.arg(i).toInt();
			if (server.argName(i) == "moduleNum")
				e1c.e1config.moduleNum = server.arg(i).toInt();
			if (server.argName(i) == "universe")
				e1c.e1config.universe = server.arg(i).toInt();
			if (server.argName(i) == "dmxGamma")
				dmxGammaPerc = server.arg(i).toInt();
			if (server.argName(i) == "dmxEnabled")
			{
				e1c.e1config.DMXoutputOn = true;
				dmx_serial_init();
			}
			/*
			if (server.argName(i) == "OledEnabled")
			{
				e1c.e1config.OLEDoutputOn = true;
				OLED_init();
			}
			*/
			if (server.argName(i) == "x_DxmPwrOffTime")
				e1c.e1config.DxmPowerOffTime = server.arg(i).toInt();

			if (server.argName(i) == "x_LEDconf")
			{
				e1c.e1config.LEDconfig = server.arg(i).toInt();
				Battery_Init();
				Led_Init();
			}
		...
		TempDeviceName = "Ucl-";
		TempDeviceName += (String)DMX_Actor[e1c.e1config.deviceNum];
		if (e1c.e1config.deviceNum < 5)
			e1c.e1config.moduleNum = 0;
		else
		{
			TempDeviceName += "-";
			TempDeviceName += (String)DMX_Module[e1c.e1config.moduleNum];
		}
		DEBUGGING(TempDeviceName);
		e1c.e1config.DeviceName = TempDeviceName;
...
void send_DMX_configuration_values_html()
{
	...
	values += "devicename|" + (String)e1c.e1config.DeviceName + "|div\n";
	values += "x_firmware|" + (String)e1c.e1config.Firmware + "|div\n";

	values += "deviceNum|" + (String)e1c.e1config.deviceNum + "|input\n";
	values += "moduleNum|" + (String)e1c.e1config.moduleNum + "|input\n";
	values += "universe|" + (String)e1c.e1config.universe + "|input\n";

	values += "x_dmxCh|" + (String)dmxCh + "|div\n";
	values += "x_dmxStart|" + (String)e1c.e1config.DMXstartChannel + "|div\n";
	values += "dmxGamma|" + (String)dmxGammaPerc + "|input\n";
	values += "dmxEnabled|" + (String)(e1c.e1config.DMXoutputOn ? "checked" : "") + "|chk\n";
	values += "x_DxmPwrOffTime|" + (String)e1c.e1config.DxmPowerOffTime + "|input\n";

	values += "x_LEDconf|" + (String)e1c.e1config.LEDconfig + "|input\n";
	values += "OledEnabled|" + (String)(e1c.e1config.OLEDoutputOn ? "checked" : "") + "|chk\n";
